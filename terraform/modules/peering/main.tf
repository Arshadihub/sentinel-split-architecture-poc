...
resource "aws_route" "gateway_to_backend" {
  count                     = length(var.gateway_private_rtb_ids)
  route_table_id            = var.gateway_private_rtb_ids[count.index]
...
resource "aws_route" "backend_to_gateway" {
  count                     = length(var.backend_private_rtb_ids)
  route_table_id            = var.backend_private_rtb_ids[count.index]
